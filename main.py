...
client = commands.Bot(command_prefix='l.',help_command = None) # modify the prefix here 
...
async def on_ready(): # When the bot is online
    print("Bot Ready !")
    await client.change_presence(activity = discord.Activity(type = discord.ActivityType.listening, name = "Surveille les membre du serveur !")) # "Listening to Cool Music !" < in status
...
async def dice(ctx,nb:int):
    ...
    embed=discord.Embed(
        ...
        description = f"Le dé tombe sur `{result}` l. ",
...
async def help(ctx):
    ...
    embed.add_field(
        ...
        value="Faîtes `l.dice <number of face>` ou encore `l.dé <nombre de faces>`",
        inline = True
    )
    embed.add_field(
        name = "Pour bannir quelqu'un",
        value = "Faîtes `l.ban @user <nombre de jours pour supprimer les messages> <Raison>`",
        inline = True
    )
    embed.add_field(
        name = "Pour kick quelqu'un",
        value = "Faîtes `l.kick @user <Raison>`",
        inline = True
    )
...
client.run("ODg5NjEwMzAwOTM0OTc5NjMz.YUjwJw.aqEu8nc22mItWg21TQIK0RY8spY")